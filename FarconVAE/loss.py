...
def kld_exact(mu_p, ss_p, mu_q, ss_q):
    ...
    # if torch.isnan(mu_p).any() or torch.isnan(ss_p).any() or torch.isnan(mu_q).any() or torch.isnan(ss_q).any():
    #     print('invalid vals')
    # if torch.isinf(mu_p).any() or torch.isinf(ss_p).any() or torch.isinf(mu_q).any() or torch.isinf(ss_q).any():
    #     print('invalid inf vals')
...
class FarconVAELoss(nn.Module):
    def __init__(self, args, device, total_train_it):
        ...
        self.recon_lossf = nn.BCELoss() #if args.data_name != 'SCF_Race' else nn.CrossEntropyLoss()   # recon(z, dec(enc(z))) # Change for scf
    ...
    def forward(self, out1, out2, x_ori, x_cont, s_ori, s_cont, y, model, current_iter, is_train=True):
        if is_train:
            ...
            recon_x_loss1, recon_x_loss2 = self.recon_lossf(out1[1][0], x_ori), self.recon_lossf(out2[1][0], x_cont) # For Z_x
            recon_s_loss1, recon_s_loss2 = self.recon_lossf(out1[1][1], s_ori.float()), self.recon_lossf(out2[1][1], s_cont.float()) # For Z_s
